...
const ODApproval = () => {
  ...
  useEffect(() => { load(); }, []);
  const load = async () => { setLoading(true); try { const r = await api.get<any[]>('/api/hod/od/pending'); if (r.success) setOds(r.data); } catch { } setLoading(false); };

  const handleAction = async (id: string, action: 'approve' | 'reject') => {
    try { await api.post(`/api/hod/od/${id}/${action}`); load(); } catch { }
  };

  return (
    <div>
      <PageHeader title="OD Approval" description="Review on-duty requests (HOD final approval)" />
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? <tr><td colSpan={5} className="p-8 text-center"><Loader2 className="w-5 h-5 animate-spin mx-auto text-muted-foreground" /></td></tr> :
              ods.length === 0 ? <tr><td colSpan={5} className="p-8 text-center text-muted-foreground"><Workflow size={32} className="mx-auto mb-2 opacity-30" />No pending OD requests</td></tr> :
                ods.map(o => (
